...
func test_keystore() {
	...
	ks := NewHDKeyStore("./data/" + account.Address.Hex())

	err = ks.StoreKey(ks.KeysDirPath, key, "456")
	if err != nil {
...
func test_keystore2() {
	...
	ks := NewHDKeyStore("./data/")
	fmt.Println(ks)
	//ks.JoinPath(account.Address.Hex())
	err = ks.StoreKey(ks.JoinPath(account.Address.Hex()), key, "456")
	if err != nil {
		log.Panic("failed to StoreKey:", err)
	}
